...
- **haberler = elciler-tam** (2026-09-26): kullanıcı Merkür kolajını "çok kötü" bulup geri aldırdı (revert 10fe6ef). Yeni kural: Elçiler TAM tablo, arka plan/raf/kafatası korunur; ince Floyd–Steinberg 704×694, sert eşik yok, çok hafif tarama satırı kayması. ESER alanları `w`, `h`, `tam = true` → tuval tablonun oranında, `--tuval-max: w px` (1:1, moiré yok); <800 px yumuşak ölçekleme. Betik scratchpad `sanat/elciler.html`, `elciler-surucu.mjs`, `e2.json`. Ders: 1-bit görseli ekrandaki boyutuyla birebir üret; küçültme moiré yapar.
